...
Os catálogos **`bakehouse_dev` e `bakehouse` já existem** — criados antes do workshop com:

```sql
CREATE CATALOG IF NOT EXISTS bakehouse_dev;
CREATE CATALOG IF NOT EXISTS bakehouse;   -- para o fecho em prod (ver §5)
```
...
**Validado ponta a ponta neste desenho (16/07/2026 — Run 5):** o build completo — 15 tabelas +
job + dashboard, um schema por camada — rodou inteiro no target `dev`. O que ficou provado:

- **`presets` no lugar de `mode: development`** entrega os schemas com nome literal
  (`bakehouse_dev.bronze`/`silver`/`gold`) e o job já nasce **PAUSED**. É a correção descrita
  logo abaixo, agora confirmada no spec deployado.
- **`${resources.schemas.X.name}` dentro do `configuration:` do pipeline resolve.** O SQL usa
  `${medallion_catalog}.${silver_schema}.customers` e o recurso de schema vira fonte única de
  verdade — renomeou o recurso, o SQL segue junto.
- **Não se assuste no telão:** esse `${resources.schemas.X.name}` aparece **não resolvido** no
  `databricks bundle validate -o json`. É adiamento do Terraform, **não** é bug — no spec
  deployado chega `"bronze_schema":"bronze"`.
- **Dashboard com `dataset_catalog` + `dataset_schema`** (CLI ≥ 0.281.0; aqui 1.1.0) + nomes de
  tabela **sem qualificar** no `.lvdash.json` → o mesmo arquivo serve `dev` e `prod`. Mata o bug
  "catálogo hard-coded" da tabela de bugs.
- **Tempos do lado Databricks:** deploy ~35 s · run do pipeline (full refresh, 15 tabelas, com
  `ai_analyze_sentiment`) ~100 s · run do job ~70 s.
- **Conferências que pegaram coisa:** as 5 marts de receita fecham com a silver em **US$ 66.471,00**
  exatos; rodar o job de novo **não duplica** nada (as streaming tables retomam incremental). O bug
  "MV que faz JOIN com MV do mesmo pipeline materializa vazia" **não** reapareceu
  (`gold.sentiment_by_franchise` veio com as 48 franquias no 1º refresh) — ou seja, é
  **intermitente**, não determinístico. Continue conferindo as contagens da gold em toda run.
...
| --- | --- | --- |
| **`ai_analyze_sentiment` tem 4 categorias** (`positive`/`negative`/`neutral`/**`mixed`**): `mixed` é **~1/3** dos reviews e `neutral` **não aparece** | **Silencioso** — sem erro; ~1/3 dos reviews some do gold | Runs 1 e 5 |
...
| **MV que faz JOIN com outra MV do mesmo pipeline** materializa vazia no 1º refresh | **Silencioso** — pipeline reporta SUCCESS com 0 linhas; `full refresh` isolado resolve. **Intermitente**: não reapareceu na Run 5 | Run 2 |
| **Dashboard com catálogo/schema hard-coded** no JSON | Cosmético — quebra ao trocar de target. Fix: `dataset_catalog`/`dataset_schema` + nomes sem qualificar | Run 1 |
| **`marker-map` (mapa lat/long) não tem spec JSON documentado** — a skill só cita o nome na tabela "Other Visualization Types" | Deploy **verde**, widget quebrado: *"Visualization has no fields selected"*. **Engana**: esse erro normalmente é `fields[].name` ≠ `encodings.fieldName`, mas aqui os nomes batem — são as **chaves** de encoding que o widget não conhece. A API guarda o JSON do widget **sem validar**. Saída documentada: **`scatter` v3** (x=longitude, y=latitude, size=receita, color=país) — sem basemap. `choropleth-map` **não** salva: a skill documenta `admin0` como `"type": "value"` com nome **fixo**, então usá-lo como campo é a mesma extrapolação | Run 5 |
...
**Números do dataset — gabarito para conferir no fim** (independem do desenho; batem exato nas
Runs 1–5): receita total **US$ 66.471,00**, ticket médio **US$ 19,94**, **3.333** pedidos,
**48** franquias, **9** países, **204** reviews, **6** produtos.

⚠️ **O split de sentimento NÃO é gabarito — ele desliza entre runs.** `ai_analyze_sentiment` é
função de LLM, logo **não é determinística**: a mesma tabela deu `86 neg / 69 mixed / 49 pos`
numa validação e `85 neg / 68 mixed / 51 pos / 0 neutros` na Run 5. O **total** (204) fecha
sempre; a divisão, não. Se você conferir os números exatos no palco vai achar que quebrou.

**Bug real que a validação pegou:** `ai_analyze_sentiment` devolve `positive/negative/neutral/
mixed`, e a primeira versão do gold não contava `mixed` — deixando ~1/3 dos reviews de fora.
Duas correções de fato sobre o que se dizia aqui antes: **`mixed` nunca foi "a maioria"** (é
~33%; quem lidera é `negative`, ~42%) — e o mais interessante para o palco é que **`neutral` deu
ZERO**. A categoria que todo mundo lembra não aparece nunca; a que ninguém lembra vale um terço
dos dados. **Lição:** rode o pipeline e olhe os dados antes de plugar o dashboard — é exatamente
o que a validação obrigatória da skill de dashboard força.
